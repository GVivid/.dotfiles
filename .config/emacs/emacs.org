...
Makes the clipboard work between emacs and the rest of the computer. It will probably break if on wayland.
#+BEGIN_SRC emacs-lisp
;;; XCLIP
...
This is where I make sure my themes folder is included.
#+BEGIN_SRC emacs-lisp
...
I want to synchronize my org-agenda to my caldav server for access on other devices.
This does not currently work.
#+NAME: Configure-CalDav
#+BEGIN_SRC emacs-lisp :tangle no
(use-package org-caldav
:ensure t
:defer t
:init
(defun my/org-caldav-load-authinfo ()
  "Extract Radicale server parameters from .authinfo."
  (when-let ((auth (car (auth-source-search :machine "caldav-server"
                                            :require '(:user :secret)))))
    (setq org-caldav-url (plist-get auth :url)
          org-caldav-calendar-id (plist-get auth :calendar-id))))
...
(my/org-caldav-load-authinfo)
(advice-add 'org-caldav-sync :before #'my/org-caldav-load-authinfo)

;; 1. Automatically create :ID: properties on Org headings
(require 'org-id)
(setq org-id-link-to-org-use-id 'create-if-interactive-and-no-custom-id)

;; File paths
(setq org-caldav-inbox (expand-file-name "CalDav-Inbox.org" org-directory)
      org-caldav-save-directory (expand-file-name ".org-caldav/" org-directory))

;; 2. Include BOTH Agenda and Inbox in org-caldav-files so synced inbox items are tracked
(setq org-caldav-files (list (expand-file-name "CalDav-Agenda.org" org-directory)
                             org-caldav-inbox))
...
;; -------------------------------------------------------------
;; VTODO (Tasks.org) Synchronization
;; -------------------------------------------------------------
(setq org-caldav-sync-todo t
      org-caldav-todo-deadline-schedule-property t)

;; Radicale settings
(setq org-caldav-sync-past 365
      org-caldav-sync-future 365
...
I have an Obsidian Vault that I would like to be compatible with emacs. In theory, I will transition to org-mode in the future, but I primarily use Obsidian, mostly due to mobile sync. Once I figure out how I want to use org-mode, and what to do with the Obsidian vault, then I can easily turn this off or just use it for mobile sync.
#+BEGIN_SRC emacs-lisp :noweb yes
...
** Custom Monodeep theme
:PROPERTIES:
:header-args: :mkdirp yes :tangle themes/monodeep-theme.el
:END:

I have a theme for my neovim config, so I would like to make it work for emacs, but it is not working very well yet.
#+BEGIN_SRC emacs-lisp :tangle no
;;; monodeep-theme.el --- Emacs theme based on the Monodeep Neovim theme -*- lexical-binding: t; -*-

;;; Code:

(deftheme monodeep
  "Monodeep dark color theme.")

(let ((class '((class color) (min-colors 89)))
	  ;; Palette mapping from the Vim theme
	  (black              "#212121")
	  (medium_gray        "#767676")
	  (white              "#F3F3F3")
	  (actual_white       "#FFFFFF")
  ...
  (custom-theme-set-faces
   ...
   ;; Basic / General Faces
   `(default ((t (:background ,asphalt :foreground ,clouds))))
   `(cursor ((t (:background ,color3 :foreground ,asphalt))))
   `(shadow ((t (:foreground ,medium_gray))))
   `(region ((t (:background ,alt_asphalt))))
   `(highlight ((t (:background ,alt_asphalt))))
   `(hl-line ((t (:background ,asphalt_subtle))))
   `(fringe ((t (:background ,asphalt_subtle :foreground ,alt_asphalt))))
   `(linum ((t (:background ,asphalt_subtle :foreground ,alt_asphalt))))
   `(line-number ((t (:background ,asphalt_subtle :foreground ,alt_asphalt))))
   `(line-number-current-line ((t (:background ,asphalt_subtle :foreground ,color3 :weight bold))))

   ;; Syntax / Font Lock Faces
   `(font-lock-builtin-face ((t (:foreground ,color0))))
   `(font-lock-comment-face ((t (:foreground ,medium_gray :slant italic))))
   `(font-lock-constant-face ((t (:foreground ,alt_color2))))
   `(font-lock-doc-face ((t (:foreground ,medium_gray :slant italic))))
   `(font-lock-function-name-face ((t (:foreground ,color4))))
   `(font-lock-keyword-face ((t (:foreground ,color0))))
   `(font-lock-negation-char-face ((t (:foreground ,color5))))
   `(font-lock-preprocessor-face ((t (:foreground ,alt_color1))))
   `(font-lock-regexp-grouping-backslash ((t (:foreground ,color5))))
   `(font-lock-regexp-grouping-construct ((t (:foreground ,color5))))
   `(font-lock-string-face ((t (:foreground ,alt_color2))))
   `(font-lock-type-face ((t (:foreground ,color3))))
   `(font-lock-variable-name-face ((t (:foreground ,color4))))
   `(font-lock-warning-face ((t (:foreground ,color2 :weight bold))))

   ;; UI & Mode Line
   `(mode-line ((t (:background ,asphalt :foreground ,clouds :box (:line-width 1 :color ,asphalt_subtle)))))
   `(mode-line-inactive ((t (:background ,asphalt :foreground ,medium_gray :box (:line-width 1 :color ,asphalt_subtle)))))
   `(vertical-border ((t (:foreground ,asphalt_subtle))))
   `(minibuffer-prompt ((t (:foreground ,color0 :weight bold))))

   ;; Search & Matching
   `(isearch ((t (:background ,color0 :foreground ,clouds))))
   `(isearch-lazy ((t (:background ,asphalt :foreground ,clouds))))
   `(query-replace ((t (:background ,color0 :foreground ,clouds))))
   `(show-paren-match ((t (:background ,asphalt_subtle :foreground ,color4 :weight bold))))
   `(show-paren-mismatch ((t (:background ,alt_color0 :foreground ,asphalt :weight bold))))

   ;; Errors & Warnings
   `(error ((t (:foreground ,alt_color0 :weight bold))))
   `(warning ((t (:foreground ,color2 :weight bold))))
   `(success ((t (:foreground ,color1 :weight bold))))

   ;; Outline / Headers
   `(outline-1 ((t (:foreground ,alt_color5 :weight bold))))
   `(outline-2 ((t (:foreground ,alt_color2 :weight bold))))
   `(outline-3 ((t (:foreground ,alt_color0 :weight bold))))
   `(outline-4 ((t (:foreground ,color4))))
   `(outline-5 ((t (:foreground ,color0))))
   `(outline-6 ((t (:foreground ,color2))))

   ;; Completion / Ivy / Helm / Company
   `(completions-common-part ((t (:foreground ,alt_color3))))
   `(company-tooltip ((t (:background ,asphalt_subtle :foreground ,clouds))))
   `(company-tooltip-selection ((t (:background ,alt_color0 :foreground ,clouds))))
   `(company-tooltip-scrollbar-track ((t (:background ,asphalt))))
   `(company-tooltip-scrollbar-thumb ((t (:background ,medium_gray))))

   ;; Diff & Git
   `(diff-added ((t (:foreground ,color1))))
   `(diff-removed ((t (:foreground ,color0))))
   `(diff-changed ((t (:foreground ,alt_color2))))
   ))
;;;###autoload
(provide-theme 'monodeep)
;;; monodeep-theme.el ends here
...
This is just a couple of the initializations.
#+NAME: Initialization
#+BEGIN_SRC emacs-lisp
