...
#+NAME: Gnuplot-Path-Config
#+BEGIN_SRC emacs-lisp
"~/Documents/org-babel-output/"
#+END_SRC
...
This is where I set up my gnuplot config.
I want to be able to take notes using mostly plain text, but graphs are a big problem.
So, I am using gnuplot to create images from text, which reduces the amount of storage space needed to "store" the graphs, and allows me to fuzzy find through all my files more easily.
#+NAME: Gnuplot-Config
#+BEGIN_SRC emacs-lisp :noweb yes
  ...
  (defun my/org-babel-gnuplot-path-fix (info)
    ...
    (let* ((lang (nth 0 info)) ;; Set lang = first argument of babel heading, aka, the language in the block.
           (params (nth 2 info)) ;; Set params = rest of params. nth 1 is the code block.
           (file-assoc (assoc :file params)) ;; extract :file param.
           (output-dir <<Gnuplot-Path-Config>>)) ;; output-dir
      
      (when (and (string= lang "gnuplot") file-assoc) ;; check if lang is gnuplot and there is a file-assoc
